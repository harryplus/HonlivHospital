...
public class GalleryItem extends Gallery {
    ...
    public ArrayList<String> arraylist;  //每个item下面的文字
    public List<String> imageUrlList;  //每个图片的url
    ...
    public GalleryItem(Context context, OrderInfo orderinfo) {
        ...
        imageUrlList = orderinfo.getPics();
    }
...
//			return Integer.MAX_VALUE;
        ...
        public View getView(int position, View view, ViewGroup viewgroup) {
            View retval = LayoutInflater.from(mContext).inflate(
                    R.layout.gallery_layout_item, null);

            ImageView retIV = (ImageView) retval.findViewById(R.id.galleryitem_IV);
            Glide.with(mContext).load(Utils.checkImagUrl(Utils.checkImagUrl(imageUrlList.get(position/* % orderInfo.getPics().size()*/)))).crossFade().placeholder(R.mipmap.picture_no).into(retIV);
